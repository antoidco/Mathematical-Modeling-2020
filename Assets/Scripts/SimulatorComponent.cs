...
public class SimulatorComponent : MonoBehaviour {
    ...
    void Start() {
        _aircraft = new Aircraft(
            new System.Numerics.Vector3(AircraftInstance.transform.position.x, AircraftInstance.transform.position.y,
                AircraftInstance.transform.position.z), new Rotation());
        var engine1 = new Engine(10, "Engine 1", System.Numerics.Vector3.Zero);
        _aircraft.Components.Add(engine1);
        engine1.CurrentPower = 400;

        _simulator = new Simulator(_aircraft,
            new Weather(new Wind(new ConstantWindModel(new System.Numerics.Vector3(0, 0, 0)))));
    }
